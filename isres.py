...
def convertToBytes(value, unit):
	...
	if (unit=='k' or unit=='K'):
		...
		if (self.verbose >= VERBOSE_LEVEL_LOW):
			print 'ERROR: Wrong unit parameter: "%s"!' % unit;
...
def measure(commandLine, outputTime, outputMem, verbose, samplePeriod):
...
def main():
	...
	if (len(sys.argv) < 3):
		print 'Usage:';
		print '\t' + sys.argv[0] + ' OUTPUT_PREFIX COMMAND [PARAMETERS]';
		exit(1);
		
	commandWithParameters = ' '.join(sys.argv[2:]);
	measure(commandWithParameters, sys.argv[1] + '.tme', sys.argv[1] + '.mem', verboseLevel, samplePeriod);
